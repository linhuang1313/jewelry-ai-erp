...
### 第十一次对话完成的功能与修复（2026-02-26）

#### 入库确认/反确认全面修复（9 项）
- [x] `KeyError 'type'` in `generate_context_summary` 导致 chat-stream 500 错误修复
- [x] 批量入库 `extract barcode from item before use` 修复
- [x] AI 退货提示防止客户名幻觉（`sales return prompt` 加约束）
- [x] 入库确认 `null guard for supplier stats` 防护
- [x] 入库单软删除功能（draft only，保留 30 天）
- [x] `Decimal vs float` 类型错误修复（confirm/unconfirm 中 weight 运算）
- [x] `StaleDataError` 修复 — 供应商统计聚合 + `account_payables` 改 bulk SQL UPDATE
- [x] 库存重复行自动合并（`auto-merge duplicate inventory/location_inventory` during confirm/unconfirm）
- [x] 供应商汇总改用 `supplier_id` 聚合（替代 name，防止同名供应商冲突）

#### 收料单权限与打印修复（4 项）
- [x] 收料 API 传递 `X-User-Role` header 防 403
- [x] `QuickReceiptModal` 添加 `user_role` query param（nginx 代理兼容）
- [x] `App.jsx` 收料处理（`handleQuickReceipt`/`handleConfirmReceipt`）添加 `user_role`

#### 收料单打印重构（2 项）
- [x] 重新设计收料单打印布局（匹配公司 XLS 模板）
- [x] 适配 241mm × 140mm 针式打印机纸张

#### 转移单增强（5 项）
- [x] 转移单表单搜索式下拉（search input + dropdown 替代 select）
- [x] 转移单 `Decimal vs float` 类型错误修复（`create_transfer_order`）
- [x] 库存详情展开行 per-product 按需加载
- [x] 转移单项目表显示 `product_code` + 自动迁移数据库列
- [x] 转移表单支持按 `product_code` 搜索

#### 展厅库存增强
- [x] 展厅导入创建 `InboundDetail` 记录（per-barcode 明细展示）
- [x] 展厅库存详情展开回退到 `product_codes` 表

#### 业务员与数据清理
- [x] `SalespersonResponse.create_time` 改 Optional（处理 NULL 值）
- [x] `QuickOrderModal` 修复重复 React keys + 业务员 fetch 错误处理
- [x] 远程数据库修复重复业务员 ID（7 条记录，id 1-7 各有 2 条）
- [x] 远程数据库修复 8 张表重复 ID（196 条记录重新分配）

#### 项目清理
- [x] 清理非项目文件（40 个一次性调试/修复脚本）
- [x] 移除未使用的 `.minimax` skills 目录
- [x] 移除 design studio 和测试文件

#### 安全加固（修复之前的技术债务）
- [x] `fbl_finance.py` 25 个写入端点添加 `require_permission` 权限检查（P0 技术债务清零）
- [x] `loan.py` 7 个写入端点添加 `require_permission` 权限检查（P1 技术债务清零）
- [x] 修复 4 处 f-string SQL 注入（`fbl_finance.py`，P1 技术债务部分清零）

#### README 重写
- [x] 完整重写 `README.md`，包含项目概览、技术栈、功能列表、部署说明

### 第十二次对话完成的功能与修复（2026-02-27 ~ 2026-02-28）

#### F-code 快捷开单自动填充（6 项）
- [x] F-code（一码一物）商品编码自动填充商品名、克重、销售工费
- [x] 商品编码失焦自动匹配 → 填充商品名（`onBlur` + `searchCode`）
- [x] F-code 超过 `limit=100` 时回退到后端 API（`/api/inventory/by-code`）
- [x] 快捷开单模态框显示 F-code 入库明细面板（16 个镶嵌字段）
- [x] F-code 入库明细面板可编辑 + 销售工费同步到订单输入
- [x] 简化快捷开单入库明细，修复编码加载上限

#### 入库管理增强（3 项）
- [x] 入库单详情表显示全部镶嵌字段（新增 9 列：副石重、副石粒数、副石单价、主石字印、副石字印、珍珠重、轴承重、销售克工费、销售件工费）
- [x] 入库单表格水平滚动 + 列间距优化（`min-w-max` + `overflow-x-auto` + `px-3`）
- [x] 入库单编辑功能支持全部 15 个镶嵌字段（不再仅限克工费）

#### 销售管理增强（7 项）
- [x] 销售单下载改为 Excel 格式（`.xlsx`，替换原 PDF 导出）
- [x] 库存检查改用 `SUM` 聚合（修复同名商品多条 `LocationInventory` 只取 `first()` 导致误报库存不足）
- [x] F-code 销售单详情显示 10 个镶嵌字段（主石重、主石粒数、副石重、副石粒数、主石字印、副石字印、珍珠重、轴承重、销售克工费、销售件工费）
- [x] Excel 导出包含 F-code 镶嵌字段（动态列生成）
- [x] 销售单详情表添加"商品编码"列
- [x] 草稿销售单删除功能（`DELETE /api/sales/orders/{order_id}`，仅 draft 状态可删）
- [x] 合并重复 `location_inventory` 行 + 加固 `confirm_sales_order` 库存检查

#### 结算管理增强（7 项）
- [x] 选择"结料"时自动填充金料重量 = 销售单总克重（`physical_gold_weight = total_weight`）
- [x] "客户提供黄金重量" 改名为 "客人应付黄金重量"
- [x] 待开结算单展开明细面板（点击行展开，显示商品编码、名称、件数、克重、工费）
- [x] 结算单一览展开明细面板 + 付款汇总（结价/结料、金价、金额等）
- [x] F-code 结算明细显示 10 个镶嵌字段（从 `InboundDetail` 动态获取）
- [x] `SettlementPage` 所有 `fetch` 添加 `user_role=settlement` query param（修复 403）
- [x] 结算确认 500 错误修复（`confirm_settlement_order` 中 `Decimal/float` 类型显式转换）

#### @结算 提料协同（12 项，核心新功能）
- [x] `@结算` 提料复用已有提料流程（创建 `CustomerWithdrawal` + `ActionCard`）
- [x] 金料余额方向修复（`current_balance` 负值=客户存料，`abs()` 校验）
- [x] `Decimal+float` 类型错误修复（`card_executor.py` + `gold_material.py`）
- [x] 确认按钮文字改为"确认并打印"
- [x] 确认后自动打开打印页面（`window.open` 提料单打印）
- [x] 提料余额显示正数化（`ActionCardRenderer` 统一正数展示）
- [x] 提料打印隐藏 AI 相关信息
- [x] 提料创建时显式设置 `created_at`（`chat_handlers.py` + `gold_material.py`）
- [x] 结算管理新增"提料明细"Tab → 后迁移至金料管理"取料单（客人拿走）"
- [x] 提料确认流程最终确定：结算确认+扣减余额，料部仅发料
- [x] "收料"→"公司收料"、"提料"→"客人提料"按钮重命名（`zh.json` + `en.json`）
- [x] `GoldMaterialPage` 修复 nested `data.data?.withdrawals` 访问

#### 权限调整（5 项）
- [x] 结算专员获得 `can_confirm_gold_receive` + `can_complete_withdrawal` 权限
- [x] 料部移除 `can_confirm_gold_receive` + `can_complete_withdrawal` 权限
- [x] 收料确认 API 传递 `user_role` 参数（`GoldMaterialPage.tsx`）
- [x] `SettlementPage` 添加 `X-User-Role` header → 改用 query param（header 被 nginx 过滤）
- [x] `SettlementPage` 创建结算单 403 修复（所有 fetch URL 添加 `user_role=settlement`）

#### 自然语言功能扩展
- [x] 新增金料查询动作（`handle_query_gold_records`）— 收/付/取料历史查询
- [x] 聊天处理器替换硬编码"AI助手"为实际角色显示名

#### AI 架构讨论与 AI-ERP2.0 项目
- [x] 梳理当前 AI 架构（`ai_analyzer.py` God Object 问题、token 浪费）
- [x] 讨论 Multi-Agent + Skills 架构方案（Agent 注册表 + Skill/Tool 模式）
- [x] 讨论 Domain Agent + Role View + Shared Skills 商业模型（适配客户分群）
- [x] 创建 GitHub 私有仓库 `linhuang1212-coder/AI-ERP2.0`
- [x] 复制项目代码到 AI-ERP2.0 作为重构基础（排除 .git/node_modules/.venv 等）

### 第十三次对话完成的修复（2026-03-01）

#### 入库确认后转移提示卡片明细修复（3 项）
- [x] 扩展 `transferPrompt.items` 数据结构 — 从仅 `product_name`+`weight` 扩展为包含 `product_code`、`supplier`、`labor_cost`、`piece_count`、`piece_labor_cost`、`total_cost` 6 个额外字段
- [x] 修复 4 处 items 构建位置（`SystemMessage.jsx` 单个确认+批量确认、`ChatView.jsx` API 获取明细+批量确认）
- [x] `TransferPromptCard.jsx` 商品列表从简单两列改为动态表格布局（按需显示条码、克工费、件工费、供应商、总成本列）
...
- [x] `.cursor/rules/git-push-after-edit.mdc` — 每次修改后自动 git push
- [ ] 数据导入架构知识规则（两种导入脚本差异、InboundOrder.status 取值、operator 标识）
- [ ] 远程操作指南规则（Cursor 沙箱限制、Python paramiko/sqlalchemy 直连方式）

### AI 架构重构（第十二次对话新增，AI-ERP2.0 项目）
- [x] 现有架构分析：`ai_analyzer.py` God Object 问题、token 浪费、角色 Prompt 混杂
- [x] 目标架构设计：Domain Agent + Role View + Shared Skills 模式
- [x] AI-ERP2.0 GitHub 仓库创建（`linhuang1212-coder/AI-ERP2.0`，private）
- [ ] 阶段 1：角色专属 Prompt 模板隔离（从结算专员试点）
- [ ] 阶段 2：Skill 注册表 + Agent 路由器
- [ ] 阶段 3：Domain Agent 封装（批发展厅、零售店、料商等客户分群）
...
- [x] ~~`fbl_finance.py` 25 个写入接口无权限控制~~ → 已修复（第十一次对话，25 个端点添加 require_permission）
- [x] ~~`loan.py` 写入接口未使用已导入的权限检查~~ → 已修复（第十一次对话，7 个端点添加 require_permission）
...
### 安全漏洞
- [x] ~~`fbl_finance.py` 多处 f-string 拼接 SQL~~ → 已修复 4 处（第十一次对话，P1 SQL 注入风险部分清零）
...
- [ ] 异常检测（如异常大额交易、异常退货频率）
- [ ] 智能定价建议（基于历史数据和市场金价）
- [ ] AI-ERP2.0 Multi-Agent 架构重构（Domain Agent + Role View + Shared Skills，仓库已创建）
...
| AI 架构 God Object（ai_analyzer.py 承担过多职责） | 中 | `ai_analyzer.py` | 待重构（第十二次对话识别，AI-ERP2.0 项目已创建） |
| Decimal vs float 反复出现 | 中 | settlement.py/card_executor.py/gold_material.py/inbound.py/warehouse.py | 持续修复中（第九~十二次对话多次修复，需全局排查） |
| X-User-Role header 被 nginx 过滤 | 中 | 前端全局 | 部分修复（SettlementPage 改用 query param，其他页面待统一） |
...
| ~~`fbl_finance.py` 25 个写入接口无权限控制~~ | ~~P0~~ | ~~fbl_finance.py~~ | **已修复 2026-02-27（25 个端点添加 require_permission）** |
| ~~`loan.py` 写入接口未使用权限检查~~ | ~~P1~~ | ~~loan.py~~ | **已修复 2026-02-27（7 个端点添加 require_permission）** |
| ~~`fbl_finance.py` f-string SQL 注入~~ | ~~P1~~ | ~~fbl_finance.py~~ | **已修复 2026-02-27（4 处 f-string 改为参数化查询）** |
| ~~入库确认 StaleDataError~~ | ~~中~~ | ~~inbound.py~~ | **已修复 2026-02-26（供应商聚合 + bulk SQL UPDATE）** |
| ~~库存重复行（confirm/unconfirm 产生）~~ | ~~中~~ | ~~inbound.py + sales.py~~ | **已修复 2026-02-26（auto-merge duplicate rows）** |
| ~~Decimal vs float（入库/转移/结算/提料）~~ | ~~中~~ | ~~inbound.py/warehouse.py/settlement.py/card_executor.py~~ | **已修复 2026-02-26~28（多处显式 float() 转换）** |
| ~~收料 API 403 权限错误~~ | ~~中~~ | ~~QuickReceiptModal/App.jsx~~ | **已修复 2026-02-26（添加 user_role header/query param）** |
| ~~库存检查误报不足（SUM vs first）~~ | ~~中~~ | ~~sales.py~~ | **已修复 2026-02-27（改用 SUM 聚合）** |
| ~~结算确认 500 Decimal/float~~ | ~~中~~ | ~~settlement.py confirm_settlement_order~~ | **已修复 2026-02-28（显式 float() 转换）** |
| ~~结算创建 403 权限错误~~ | ~~中~~ | ~~SettlementPage.tsx~~ | **已修复 2026-02-27（user_role=settlement query param）** |
| ~~提料余额方向错误（负值=存料）~~ | ~~中~~ | ~~card_executor.py / gold_material.py~~ | **已修复 2026-02-27（abs() 校验 + 正数化显示）** |
| ~~远程数据库重复 ID（8 张表 196 条）~~ | ~~P0~~ | ~~远程 PostgreSQL~~ | **已修复 2026-02-26（ID 重新分配）** |
| ~~AI 退货提示客户名幻觉~~ | ~~中~~ | ~~ai_prompts.py~~ | **已修复 2026-02-26（prompt 加约束）** |
| ~~generate_context_summary KeyError~~ | ~~中~~ | ~~context_manager.py~~ | **已修复 2026-02-26（'type' key 防护）** |
| ~~转移提示卡片明细缺失（仅显示名称+重量）~~ | ~~中~~ | ~~TransferPromptCard.jsx / SystemMessage.jsx / ChatView.jsx~~ | **已修复 2026-03-01（扩展 items 数据结构 + 动态表格展示）** |
...
### 第十一次对话（2026-02-26）
**涉及文件：**
- `backend/app/routers/inbound.py` — 入库确认/反确认全面修复（Decimal→float、StaleDataError、supplier_id 聚合、null guard、auto-merge 重复行）+ 软删除功能
- `backend/app/routers/warehouse.py` — 转移单 Decimal→float 修复 + product_code 搜索 + 项目表显示
- `backend/app/routers/gold_material.py` — 收料单打印布局重构（匹配公司 XLS 模板 + 241mm×140mm 针式打印机）
- `backend/app/routers/inventory_maintenance.py` — 库存详情展开行 per-product 按需加载
- `backend/app/routers/fbl_finance.py` — 25 个端点添加 require_permission + 4 处 SQL 注入修复
- `backend/app/routers/loan.py` — 7 个端点添加 require_permission
- `backend/app/routers/chat.py` — generate_context_summary KeyError 修复
- `backend/app/routers/chat_handlers.py` — 批量入库 barcode 提取修复
- `backend/app/ai_prompts.py` — AI 退货提示防客户名幻觉
- `backend/app/schemas/__init__.py` — SalespersonResponse.create_time 改 Optional
- `backend/scripts/import_showroom_inventory.py` — 展厅导入创建 InboundDetail 记录
- `frontend/src/components/QuickOrderModal.tsx` — 修复重复 React keys + 业务员 fetch 错误处理
- `frontend/src/components/QuickReceiptModal.jsx` — 添加 user_role query param
- `frontend/src/App.jsx` — 收料处理添加 user_role
- `frontend/src/components/WarehousePage.tsx` — 转移单搜索式下拉 + product_code 显示
- `frontend/src/components/InventoryOverview.tsx` — 展厅库存详情展开回退到 product_codes 表
- `README.md` — 完整重写

### 第十二次对话（2026-02-27 ~ 2026-02-28）
**涉及文件：**
- `backend/app/routers/sales.py` — Excel 导出（替换 PDF）+ 镶嵌字段 + 库存 SUM 聚合 + 草稿删除 + location_inventory 合并
- `backend/app/routers/inbound.py` — 全部镶嵌字段编辑支持
- `backend/app/routers/settlement.py` — 结算明细展开（F-code 镶嵌字段 + 付款汇总）+ Decimal/float 修复
- `backend/app/routers/chat.py` — @结算 提料协同（创建 CustomerWithdrawal + ActionCard）
- `backend/app/routers/chat_handlers.py` — 提料创建 created_at 显式设置 + 金料查询动作 + 角色显示名
- `backend/app/routers/gold_material.py` — Decimal/float 修复 + 余额方向修复 + created_at 设置
- `backend/app/routers/inventory_maintenance.py` — F-code 入库明细查询（14 个镶嵌字段）
- `backend/app/services/card_executor.py` — 提料确认扣减逻辑 + Decimal/float 修复 + 余额正数化
- `backend/app/middleware/permissions.py` — 结算/料部权限互换（can_confirm_gold_receive + can_complete_withdrawal）
- `backend/app/schemas/__init__.py` — SalesDetailResponse 新增 10 个镶嵌字段
- `frontend/src/components/QuickOrderModal.tsx` — F-code 自动填充 + 入库明细面板（可编辑）
- `frontend/src/components/InboundOrdersPage.tsx` — 9 列镶嵌字段 + 水平滚动 + 全字段编辑
- `frontend/src/components/SalesOrdersPage.tsx` — Excel 导出 + 镶嵌字段 + 商品编码列 + 草稿删除
- `frontend/src/components/SettlementPage.tsx` — 结料自动填充 + 标签改名 + 展开明细 + user_role 参数 + 提料明细 Tab（后迁移）
- `frontend/src/components/GoldMaterialPage.tsx` — 默认 Tab 修复 + nested data 访问修复 + user_role 参数
- `frontend/src/components/chat/cards/ActionCardRenderer.jsx` — 提料卡片增强（确认并打印 + 余额显示 + 自动打印）
- `frontend/src/locales/zh.json` — "收料"→"公司收料"、"提料"→"客人提料"
- `frontend/src/locales/en.json` — "Receipt"→"Company Receipt"、"Withdrawal"→"Customer Withdrawal"

### 第十三次对话（2026-03-01）
**涉及文件：**
- `frontend/src/components/chat/cards/TransferPromptCard.jsx` — 商品列表从两列布局改为动态表格（按需显示条码、克工费、件工费、供应商、总成本列）
- `frontend/src/components/chat/cards/SystemMessage.jsx` — 单个确认+批量确认的 transferPrompt.items 扩展 6 个字段
- `frontend/src/pages/ChatView.jsx` — API 获取明细+批量确认的 transferPrompt.items 扩展 6 个字段
