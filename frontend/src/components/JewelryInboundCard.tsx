...
import { Check, AlertCircle, Package, Download, Printer, X } from 'lucide-react';
...
export const JewelryInboundCardComponent: React.FC<JewelryInboundCardProps> = ({
  ...
  const handleCancel = async () => {
    if (disabled || data.status !== 'pending' || !actions.onCancel) return;
    setIsCancelling(true);
    try {
      await actions.onCancel(data);
    } catch (error) {
      ...
      <div className="p-5">
        ...
        {data.status === 'pending' && <div className="flex gap-3">
          ...
          <button
            ...
            disabled={disabled || isConfirming || isCancelling || isReporting || data.status !== 'pending'}
          ...
          {actions.onCancel && (
            <button
              ...
              disabled={disabled || isConfirming || isCancelling || isReporting || data.status !== 'pending'}
              className="px-4 py-2.5 text-sm text-gray-600 hover:text-gray-800 hover:bg-gray-100 border border-gray-300 disabled:text-gray-400 disabled:border-gray-200 disabled:hover:bg-transparent disabled:cursor-not-allowed rounded-lg transition-colors duration-200 flex items-center gap-2"
            >
              {isCancelling ? (
                <>
                  <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24">
              ...
              <button
                onClick={async (e) => {
                  ...
                  try {
                    ...
                    const baseUrl = API_ENDPOINTS.API_BASE_URL || '';
              ...
              <button
                onClick={async (e) => {
                  ...
                  try {
                    ...
                    const baseUrl = API_BASE_URL || API_ENDPOINTS.API_BASE_URL || '';
