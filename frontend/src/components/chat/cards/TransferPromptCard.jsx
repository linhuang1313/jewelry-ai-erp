 ...
 * 
 * props.data: {
 *   items: [{ product_name, weight, product_code?, supplier?, labor_cost?, piece_count?, piece_labor_cost?, total_cost? }],
...
export default function TransferPromptCard({ data, onStatusChange }) {
  ...
  return (
    <div className="mt-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200/60 shadow-sm">
      ...
      {/* 商品列表 */}
      <div className="bg-white/60 rounded-lg p-3 mb-3 overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-gray-500 border-b border-gray-200">
              <th className="pb-1.5 pr-3 font-medium">商品名称</th>
              {data.items.some(i => i.product_code) && <th className="pb-1.5 pr-3 font-medium">条码</th>}
              <th className="pb-1.5 pr-3 font-medium text-right">重量</th>
              {data.items.some(i => i.labor_cost) && <th className="pb-1.5 pr-3 font-medium text-right">克工费</th>}
              {data.items.some(i => i.piece_count) && <th className="pb-1.5 pr-3 font-medium text-right">件工费</th>}
              {data.items.some(i => i.supplier) && <th className="pb-1.5 pr-3 font-medium">供应商</th>}
              {data.items.some(i => i.total_cost) && <th className="pb-1.5 font-medium text-right">总成本</th>}
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {data.items.map((item, idx) => (
              <tr key={idx}>
                <td className="py-1.5 pr-3 text-gray-700">{item.product_name}</td>
                {data.items.some(i => i.product_code) && <td className="py-1.5 pr-3 text-gray-500 font-mono text-xs">{item.product_code || '-'}</td>}
                <td className="py-1.5 pr-3 text-gray-600 font-mono text-right">{item.weight?.toFixed(2)}g</td>
                {data.items.some(i => i.labor_cost) && <td className="py-1.5 pr-3 text-gray-500 text-right">{item.labor_cost || '-'}</td>}
                {data.items.some(i => i.piece_count) && <td className="py-1.5 pr-3 text-gray-500 text-right">{item.piece_count ? `${item.piece_count}件×${item.piece_labor_cost || 0}` : '-'}</td>}
                {data.items.some(i => i.supplier) && <td className="py-1.5 pr-3 text-gray-500">{item.supplier || '-'}</td>}
                {data.items.some(i => i.total_cost) && <td className="py-1.5 text-gray-700 font-medium text-right">{item.total_cost ? `¥${item.total_cost.toFixed(2)}` : '-'}</td>}
              </tr>
            ))}
          </tbody>
        </table>
