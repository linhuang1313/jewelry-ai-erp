...
export function SupplierPage({ userRole = 'manager' }) {
  ...
  // 获取供应商列表（带重试机制 + 分页）
  const fetchSuppliers = async (page = 1, retryCount = 0) => {
    setLoading(true)
    const maxRetries = 2
    ...
    try {
      ...
      const url = `${API_ENDPOINTS.API_BASE_URL}/api/suppliers?${params.toString()}`
      ...
      if (data.success) {
        ...
        setTotalSuppliers(data.total || suppliersArray.length)
      ...
      if (retryCount < maxRetries && (error.message.includes('fetch') || error.message.includes('network'))) {
        ...
        return fetchSuppliers(page, retryCount + 1)
      }
  ...
  const handleDelete = (supplier) => {
    setConfirmDialog({
      isOpen: true,
      title: '删除供应商',
      message: `确定要删除供应商【${supplier.name}】吗？`,
      isDestructive: true,
      onConfirm: async () => {
        ...
        try {
          const response = await fetch(
            ...
            { method: 'DELETE', headers: { 'X-User-Role': userRole } }
          )
          const data = await response.json()
          if (data.success) {
            fetchSuppliers(currentPage)
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
        <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button
              onClick={() => fetchSuppliers(currentPage)}
              disabled={loading}
        ...
        {totalSuppliers > 0 && (
          <div className="px-6 py-3 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
            <span className="text-sm text-gray-500">
              共 {totalSuppliers} 个供应商，第 {currentPage}/{totalPages} 页
            </span>
            <div className="flex items-center gap-1">
              <button
                onClick={() => { setCurrentPage(1); fetchSuppliers(1) }}
              ...
              >首页</button>
              <button
                onClick={() => { const p = currentPage - 1; setCurrentPage(p); fetchSuppliers(p) }}
                disabled={currentPage <= 1 || loading}
                className="px-2 py-1 text-xs rounded border border-gray-300 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed"
              >上一页</button>
              {(() => {
                ...
                for (let i = start; i <= end; i++) {
                  pages.push(
                    <button
                      key={i}
                      onClick={() => { setCurrentPage(i); fetchSuppliers(i) }}
                      disabled={loading}
                      className={`px-2.5 py-1 text-xs rounded border ${i === currentPage ? 'bg-amber-500 text-white border-amber-500' : 'border-gray-300 hover:bg-gray-100'}`}
                    >{i}</button>
                  )
                }
                return pages
              ...
              >下一页</button>
              <button
                onClick={() => { setCurrentPage(totalPages); fetchSuppliers(totalPages) }}
                disabled={currentPage >= totalPages || loading}
                className="px-2 py-1 text-xs rounded border border-gray-300 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed"
              >末页</button>
            </div>
