    ...
    <header className="bg-white/80 backdrop-blur-xl border-b border-gray-200/60 px-3 md:px-6 py-2.5 md:py-4 
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-1.5 md:space-x-3">
          ...
          {/* 手机端导航菜单按钮 + 下拉菜单（共用 ref 防止 click-outside race condition） */}
          ...
          {currentPage === 'chat' ? (
            <div className="hidden md:flex items-center space-x-2" ref={navMenuRef}>
              ...
              {/* 结算专员 - 直接显示结算管理按钮（结算专员已移入工作台） */}
              {hasPermission(userRole, 'canCreateSettlement') &&
                !hasPermission(userRole, 'canReceiveTransfer') &&
                !hasPermission(userRole, 'canTransfer') &&
                userRole !== 'product' && userRole !== 'manager' && userRole !== 'settlement' && (
                  <button
                    onClick={() => handleNavClick('settlement')}
                    className="flex items-center space-x-1.5 px-3 py-2 rounded-xl border border-amber-200 text-amber-600 
                             hover:bg-amber-50 transition-all duration-200 font-medium text-[14px]"
                  >
              ...
              {/* 展厅角色 - 只有分仓转移时直接显示按钮（柜台角色已移入工作台） */}
              {(hasPermission(userRole, 'canReceiveTransfer') || hasPermission(userRole, 'canTransfer')) &&
                !hasPermission(userRole, 'canCreateSettlement') &&
                userRole !== 'product' && userRole !== 'manager' && userRole !== 'counter' && (
                <button
                    onClick={() => handleNavClick('warehouse')}
                    className="flex items-center space-x-1.5 px-3 py-2 rounded-xl border border-indigo-200 text-indigo-600 
                             hover:bg-indigo-50 transition-all duration-200 font-medium text-[14px]"
                >
              ...
              {/* 客户管理独立按钮 - 仅有客户权限时直接显示（柜台/结算角色已移入工作台） */}
              {userRole !== 'product' && !['counter', 'settlement'].includes(userRole) &&
                (hasPermission(userRole, 'canViewCustomers') || hasPermission(userRole, 'canManageCustomers')) &&
                !hasPermission(userRole, 'canManageSuppliers') &&
              ...
              {/* 历史回溯按钮 - 独立按钮，商品专员/工作台角色不显示（已整合到工作台中） */}
