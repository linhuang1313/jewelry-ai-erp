...
export const ROLE_PERMISSIONS = {
  ...
  counter: {
    ...
    // 协同卡片权限
    canExecuteActionCard: true,
    canCreateActionCard: false,
  ...
  product: {
    ...
    pages: ['chat', 'warehouse', 'supplier', 'return', 'product-codes', 'label-design', 'document-center'],
  },
  ...
  settlement: {
    ...
    canManageLoan: true,            // 可以管理暂借单（确认借出、归还、撤销）
    // 销退模块权限
    canSalesReturn: true,           // 可以操作销退（创建销退单、销退结算）
    // 收款权限
    canRecordPayment: true,         // 可以登记客户收款
    // 协同卡片权限
    canExecuteActionCard: true,
    canCreateActionCard: true,

    pages: ['chat', 'settlement', 'gold-material', 'customer', 'loan', 'returns', 'customer-account', 'sales-returns'],
  },
  ...
  sales: {
    ...
    // 协同卡片权限
    canExecuteActionCard: false,
    canCreateActionCard: true,

    pages: ['chat', 'customer', 'customer-account'],
  },
  ...
  finance: {
    ...
    // 销退模块权限
    canSalesReturn: true,           // 可以操作销退
    // 协同卡片权限
    canExecuteActionCard: true,
    canCreateActionCard: true,

    pages: ['chat', 'warehouse', 'settlement', 'finance', 'analytics', 'export', 'salesperson', 'customer', 'supplier', 'return', 'gold-material', 'product-codes', 'label-design', 'loan', 'document-center', 'voucher', 'customer-account', 'sales-returns'],
  },
  ...
  manager: {
    ...
    pages: ['chat', 'warehouse', 'settlement', 'finance', 'analytics', 'export', 'salesperson', 'customer', 'supplier', 'return', 'gold-material', 'product-codes', 'label-design', 'loan', 'document-center', 'finance-admins', 'customer-account', 'sales-returns'],
  }
};
