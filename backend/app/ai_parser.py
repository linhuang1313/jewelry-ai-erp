...
**重要**：请结合上面的对话记录理解用户当前的输入（上下文联想）。

【商品上下文】
- 如果用户说"这个"、"刚才的"、"那个"等指代词，请从对话记录中找到对应的商品名称
- 如果用户刚入库了某个商品，然后说"转移到展厅"，需要理解是转移刚入库的那个商品
- 如果用户刚查询了某商品库存，然后问"有多少"，应理解为查询同一商品

【客户上下文】
- 如果用户刚查询了某客户的欠款/存料，然后问"有欠料吗"或"存料呢"，应理解为查询同一客户
- 从对话记录中找到最近提到的客户名称，用于 customer_name、debt_customer_name 等字段
- 例如：用户先问"张三欠多少钱"，再问"有欠料吗"，应理解为查询张三的欠料

【供应商上下文】
- 如果用户刚查询了某供应商，然后问"欠我们多少"或"应付多少"，应理解为查询同一供应商
- 从对话记录中找到最近提到的供应商名称

【单据上下文】
- 如果用户刚查询了某入库单（RK开头），然后问"详情"或"商品列表"，应理解为查询同一入库单
- 如果用户刚查询了某销售单（XS开头），然后问"状态"或"结算情况"，应理解为查询同一销售单
- 如果用户刚查询了某转移单（TR开头），然后问"详情"或"关联单据"，应理解为查询同一转移单

【重要原则】
- 如果无法从上下文确定实体，对应字段可以设为 null，但要尽可能提取其他信息
- 优先从最近的对话记录中推断实体，而不是要求用户重复输入
...
16. **收料**：客户交料/来料（如"张老板交料5克"、"李老板存料3.5克足金9999"、"王总交了2克金料"、"小林来料100克"）
...
20. **供应商付款**：给供应商付工费款项（如"付2000给梵贝琳"、"给金源付款5000"、"梵贝琳付款3000元"、"付5000元给梵贝琳工厂"）
21. **查询转移单**：查询转移单/调拨单信息（如"查询转移单TR20260127001"、"最近的转移单"、"今天的调拨单"、"被退回的转移单"、"TR单号的关联单据"等）
22. **闲聊**：用户的问候、寒暄、感谢、闲聊等非业务相关内容（如"你好"、"谢谢"、"早上好"、"1+1等于几"、"今天天气怎么样"等）
...
- action: 用户意图，根据用户输入智能判断，可能是：
  ...
  - "收料"：客户交料/来料/存料（如"张老板交料5克"、"李老板存料3.5克足金9999"、"王总交了2克金料"、"收张老板5克"、"小林来料100克"、"客户来料50克"等）
  ...
  - "供应商付款"：给供应商付工费款项（如"付2000给梵贝琳"、"给金源付款5000"、"梵贝琳付款3000元"、"付5000元给梵贝琳工厂"等）
  - "查询转移单"：用户要查询转移单/调拨单信息，转移单号以TR开头（如"TR20260127001"、"查询转移单"、"调拨单"、"最近的调拨单"、"退回的转移单"、"关联单据"、"转移记录"、"调拨记录"等）
...
- 转移单/调拨单查询字段（仅当action为"查询转移单"时使用）：
  - transfer_order_no: 转移单号（字符串，TR开头，如"TR20260127001"）
  - transfer_status: 状态筛选（pending待接收/received已接收/rejected已拒收/pending_confirm待确认/returned已退回）
  - transfer_date_start: 开始日期（YYYY-MM-DD格式）
  - transfer_date_end: 结束日期（YYYY-MM-DD格式）
...
- supplier_payment_name: 付款供应商名称（当action为"供应商付款"时必填，如"梵贝琳"、"金源珠宝"等）
- supplier_payment_amount: 付款金额（当action为"供应商付款"时必填，数字，单位元）
- supplier_payment_method: 付款方式（当action为"供应商付款"时可选，可选值：转账/现金/支票/承兑，默认"转账"）
- supplier_payment_remark: 付款备注（当action为"供应商付款"时可选）
...
1. **意图识别要灵活**：
   ...
   - "付20克给金源珠宝"、"给金源付5克"、"付料金源珠宝10克" → action: "付料"（注意：这是付"克"，是金料）
   - "付2000给梵贝琳"、"给金源付款5000"、"梵贝琳付款3000元"、"付5000元给梵贝琳工厂"、"给供应商梵贝琳付2000" → action: "供应商付款"（注意：这是付"元"，是工费款项）
   ...
   - "查询转移单TR20260127001"、"TR20260127001"、"最近的转移单"、"今天的转移单"、"调拨单"、"查调拨单"、"调拨记录"、"转移记录"、"被退回的转移单"、"TR单号的关联单据" → action: "查询转移单"，并提取transfer_order_no字段（如有）
...
示例49（供应商付款 - 基本）：
用户输入："付2000给梵贝琳"
说明：给供应商付工费款项，注意是付"元"不是付"克"
{{
  "action": "供应商付款",
  "supplier_payment_name": "梵贝琳",
  "supplier_payment_amount": 2000,
  "products": null
}}
...
{{
  "action": "供应商付款",
  "supplier_payment_name": "金源珠宝",
  "supplier_payment_amount": 5000,
  "supplier_payment_method": "现金",
  "products": null
}}
...
{{
  "action": "供应商付款",
  "supplier_payment_name": "梵贝琳工厂",
  "supplier_payment_amount": 3000,
  "supplier_payment_method": "转账",
  "supplier_payment_remark": "付1月份的工费",
  "products": null
}}

示例52（查询转移单 - 带单号）：
用户输入："查询转移单TR20260127001"
说明：用户要查询特定转移单，提取转移单号
{{
  "action": "查询转移单",
  "transfer_order_no": "TR20260127001",
  "products": null
}}

示例53（查询转移单 - 单号单独输入）：
用户输入："TR20260127001"
说明：用户只输入了TR开头的转移单号，应该识别为查询转移单
{{
  "action": "查询转移单",
  "transfer_order_no": "TR20260127001",
  "products": null
...
示例56（查询转移单 - 按日期筛选）：
用户输入："今天的调拨记录"
说明：用户要查询今天的转移单，"调拨记录"与"转移单"同义
{{
  "action": "查询转移单",
  "transfer_date_start": "2026-01-27",
  "transfer_date_end": "2026-01-27",
  "products": null
}}

示例57（查询转移单 - 关联单据）：
用户输入："TR20260127001的关联单据是什么"
说明：用户要查询转移单的关联信息（来源单/重新发起的单）
{{
  "action": "查询转移单",
  "transfer_order_no": "TR20260127001",
  "products": null
