...
INTENT_DATA_MAP = {
    "查询库存":     ["inventory"],
    "查询入库单":   ["inbound_orders", "suppliers"],
    "查询销售单":   ["sales_orders", "customers"],
    "销售数据查询": ["sales_orders", "customers"],
    "查询客户":     ["customers"],
    "查询客户账务": ["customers", "customer_debt"],
    "查询供应商":   ["suppliers"],
    "查询转移单":   ["transfer_orders", "inventory"],
    "查询暂借单":   ["customers"],
    "查询对账单":   ["customers", "sales_orders"],
    "查询凭证":     ["customers"],
    "查询结算单":   ["customers", "sales_orders"],
}
...
class AIAnalyzer:
    ...
    def collect_all_data(self, intent: str, user_message: str, db: Session, order_no: Optional[str] = None, sales_order_no: Optional[str] = None, user_role: str = "manager", inbound_supplier: Optional[str] = None, inbound_product: Optional[str] = None, inbound_date_start: Optional[str] = None, inbound_date_end: Optional[str] = None, transfer_order_no: Optional[str] = None, transfer_status: Optional[str] = None, transfer_date_start: Optional[str] = None, transfer_date_end: Optional[str] = None, customer_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        # ========== 按意图过滤需要加载的数据类型 ==========
        needed_data = INTENT_DATA_MAP.get(intent)
        if needed_data is not None:
            allowed_data = [d for d in allowed_data if d in needed_data]
            logger.info(f"意图 '{intent}' 只加载数据类型: {allowed_data}")
        else:
            logger.info(f"意图 '{intent}' 未匹配映射，加载全部允许的数据类型: {allowed_data}")
        ...
        if 'suppliers' in allowed_data:
            ...
            if supplier_names:
                product_counts = db.query(
                    ...
                    func.count(func.distinct(InboundDetail.product_name))
                ).filter(
                    InboundDetail.supplier.in_(supplier_names)
                ).group_by(InboundDetail.supplier).all()
                product_count_map = {name: count for name, count in product_counts}
                for supplier_data in data["suppliers"]:
                    supplier_data["product_count"] = product_count_map.get(supplier_data["name"], 0)
        else:
            data["suppliers"] = []
        ...
        if 'inbound_orders' in allowed_data:
            ...
            inbound_orders = inbound_query.limit(30).all()
            data["inbound_orders"] = []
            ...
            # 批量查询所有入库单的明细（避免 N+1）
            order_ids = [o.id for o in inbound_orders]
            all_details = db.query(InboundDetail).filter(InboundDetail.order_id.in_(order_ids)).all() if order_ids else []
            details_by_order = {}
        ...
        if 'customers' in allowed_data:
            cust_query = db.query(Customer).filter(Customer.status == "active")
            if customer_name:
                cust_query = cust_query.filter(Customer.name.contains(customer_name))
            else:
                cust_query = cust_query.limit(50)
        ...
        if 'sales_orders' in allowed_data:
            sales_orders = db.query(SalesOrder).order_by(desc(SalesOrder.order_date)).limit(30).all()
            data["sales_orders"] = []
            
            # 批量查询所有销售单的明细（避免 N+1）
            sales_order_ids = [so.id for so in sales_orders]
            all_sales_details = db.query(SalesDetail).filter(SalesDetail.order_id.in_(sales_order_ids)).all() if sales_order_ids else []
            sales_details_by_order = {}
            for d in all_sales_details:
                sales_details_by_order.setdefault(d.order_id, []).append(d)
            
            for so in sales_orders:
                details = sales_details_by_order.get(so.id, [])
        ...
        if 'transfer_orders' in allowed_data:
            ...
            transfer_orders = transfer_query.limit(30).all()
            ...
            # 批量查询来源单和关联单（避免 N+1）
            transfer_ids = [o.id for o in transfer_orders]
            source_ids = [o.source_order_id for o in transfer_orders if o.source_order_id]
            
            # 来源单号映射: source_order_id -> transfer_no
            source_no_map = {}
            if source_ids:
                source_orders = db.query(InventoryTransferOrder.id, InventoryTransferOrder.transfer_no).filter(
                    InventoryTransferOrder.id.in_(source_ids)
                ).all()
                source_no_map = {sid: sno for sid, sno in source_orders}
            
            # 关联新单映射: source_order_id -> transfer_no
            related_no_map = {}
            if transfer_ids:
                related_orders = db.query(InventoryTransferOrder.source_order_id, InventoryTransferOrder.transfer_no).filter(
                    InventoryTransferOrder.source_order_id.in_(transfer_ids)
                ).all()
                related_no_map = {src_id: tno for src_id, tno in related_orders}
            
            for order in transfer_orders:
                source_transfer_no = source_no_map.get(order.source_order_id) if order.source_order_id else None
                related_transfer_no = related_no_map.get(order.id)
                ...
                data["transfer_orders"].append({
                    ...
                    "related_transfer_no": related_transfer_no,  # 关联新单号（被重新发起后产生的）
                    "items": [
                        {
    ...
    def format_data_for_ai(self, data: Dict[str, Any]) -> str:
        ...
        if len(text) > MAX_PROMPT_CHARS:
            logger.warning(f"数据文本过长 ({len(text)} 字符)，截断至 {MAX_PROMPT_CHARS} 字符")
            text = text[:MAX_PROMPT_CHARS] + "\n\n[数据已截断，以上为部分数据]"
        
        return text
    
    def _build_analysis_prompt(self, user_message: str, intent: str, data: Dict[str, Any], language: str = "zh") -> tuple:
        """构建 AI 分析的 prompt 和 system_prompt（analyze 和 analyze_stream 共用）
        
        Returns:
            (prompt, system_prompt, is_inbound_list_query) 三元组
        """
...
"""
        ...
        system_prompt = "你是珠宝ERP系统AI助手，请专业、准确地回答用户问题。严格基于系统提供的数据回答，不要编造任何不存在的数据、功能或界面元素。回答要简洁高效：如果没有查到相关数据，直接说明即可，不要展开分析或罗列空数据。"
...
如果没有符合条件的入库单，友好地告知用户。"""
            system_prompt = "你是珠宝ERP系统AI助手。对于入库单列表查询，必须按要求格式展示每个商品，包括入库单号、克重、供应商名称和入库日期。只展示系统提供的数据，不要编造任何单据或数字。"
...
请用自然、专业且友好的语言回答。**克重信息是珠宝行业的核心指标，必须优先展示！必须严格按照系统提供的 cash_debt 和 net_gold 数值显示账务状态！**"""
            system_prompt = "你是珠宝ERP系统AI助手，专门帮助用户查询客户账务信息。请以友好、清晰的方式回答。克重是珠宝行业最重要的指标，必须优先展示！必须严格按照系统提供的数值显示账务状态！不要编造任何数据。"
        elif intent == "销售数据查询":
...
金额格式：如果超过1万，用"万"为单位，如 ¥12.5万。"""
            system_prompt = "你是珠宝ERP系统AI助手，专门帮助用户分析销售数据。请以友好、清晰的方式回答，突出关键指标。只使用系统提供的数据，不要编造数字。"
        else:
...
请用简洁、专业的语言给出结论："""
                system_prompt = "你是珠宝ERP分析师。系统将自动生成可视化图表展示详细数据，你只需给出简洁的文字结论（150字以内），不要输出表格，重点是洞察和建议。严格基于提供的数据分析，不要编造数字。"
            elif needs_analysis:
                prompt = f"""**用户问题：** {user_message}
**用户意图：** {intent}
...
请给出专业、全面、有价值的分析报告。可以使用表格、列表等格式使信息更清晰。"""
                system_prompt = "你是珠宝ERP系统的专业数据分析师。用户明确要求分析，你必须提供详细、专业、全面的分析报告，包括数据概览、详细分析、趋势洞察和建议。不要给简短回答。严格基于系统提供的数据分析，不要编造任何数字或趋势。"
            else:
                prompt = f"""**用户问题：** {user_message}
**用户意图：** {intent}
...
现在请简短回答："""
                system_prompt = "你是珠宝ERP助手。这是简单查询，必须用50字以内直接回答，不要分析，不要建议，不要表格。只使用系统提供的数据，不要编造。"
        ...
        return prompt, system_prompt, is_inbound_list_query
    
    def _append_export_marker(self, content: str, is_inbound_list_query: bool, data: Dict[str, Any]) -> str:
        """如果是入库单列表查询，追加导出标记"""
        if is_inbound_list_query and data.get("inbound_orders"):
            ...
            return content + export_marker
        return content
    
    def analyze(self, user_message: str, intent: str, data: Dict[str, Any], knowledge_base: str = None) -> str:
        """使用 AI 进行智能分析和计算"""
        
        # 系统帮助：使用知识库回答
        if knowledge_base and intent in ["系统帮助", "闲聊"]:
            system_prompt = f"""你是珠宝ERP系统AI助手。以下是系统操作手册，请根据用户的问题给出准确、友好的回答。

**反幻觉规则（必须严格遵守）：**
1. 只能回答操作手册中明确记载的内容，不要编造任何功能、按钮、菜单或操作流程
2. 如果手册中没有相关信息，请诚实回答"抱歉，我暂时没有这方面的信息"
3. 不要提及任何不存在的UI元素（如"帮助按钮"、"设置菜单"等），除非手册中明确提到
4. 导航栏的实际结构：左侧有功能模块菜单，右侧只有语言切换和角色切换，没有"帮助"按钮
5. 回答时引用手册中的具体章节，确保信息可追溯
...
{knowledge_base}"""
            try:
                ...
                logger.error(f"系统帮助AI分析失败: {e}", exc_info=True)
                return f"抱歉，获取帮助信息时出现错误：{str(e)}。请稍后重试。"
        
        prompt, system_prompt, is_inbound_list_query = self._build_analysis_prompt(user_message, intent, data)
        
        total_chars = len(system_prompt) + len(prompt)
        est_tokens = int(total_chars * 0.85)
        logger.info(f"[Token估算] 意图={intent}, system_prompt={len(system_prompt)}字符, prompt={len(prompt)}字符, 总计≈{est_tokens}tokens")
        
        if est_tokens > 110000:
            logger.warning(f"[Token超限风险] 预估{est_tokens}tokens超过限制，截断prompt")
            max_prompt_len = int(110000 / 0.85) - len(system_prompt)
        ...
        try:
            ...
            return self._append_export_marker(ai_content, is_inbound_list_query, data)
        except Exception as e:
    ...
    def analyze_stream(self, user_message: str, intent: str, data: Dict[str, Any], language: str = "zh", knowledge_base: str = None):
        """使用 AI 进行流式分析和计算（生成器函数）"""
        
        # 系统帮助：使用知识库回答（流式）
        if knowledge_base and intent in ["系统帮助", "闲聊"]:
            kb_system_prompt = f"""你是珠宝ERP系统AI助手。以下是系统操作手册，请根据用户的问题给出准确、友好的回答。
...
{knowledge_base}"""
            ...
            try:
                ...
                logger.error(f"系统帮助流式AI分析失败: {e}", exc_info=True)
                yield f"抱歉，获取帮助信息时出现错误：{str(e)}。请稍后重试。"
                return
        
        # 复用统一的 prompt 构建方法
        prompt, system_prompt, is_inbound_list_query = self._build_analysis_prompt(user_message, intent, data, language)
        ...
        try:
            ...
            export_marker = self._append_export_marker("", is_inbound_list_query, data)
            if export_marker:
                yield export_marker
            ...
            try:
                ...
                full_text = self._append_export_marker(full_text, is_inbound_list_query, data)
