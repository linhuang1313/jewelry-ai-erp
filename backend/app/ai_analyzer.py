...
请基于这些数据，进行深入分析和计算，全面回答用户的问题。要求：

1. **全面分析**：从多个维度进行分析（如总工费、总重量、商品种类、供货频次、客户价值、销售趋势等）
2. **智能计算**：进行必要的数值计算和统计分析
3. **智能判断**：基于数据做出合理的判断和结论
4. **详细说明**：解释为什么得出这个结论，列出关键数据支撑
5. **业务洞察**：提供有价值的业务建议和洞察
6. **格式清晰**：使用清晰的格式，便于阅读

请用自然、专业的语言进行分析，直接回答用户的问题。如果用户问的是"谁最重要"、"哪个最好"这类问题，请明确说明判断依据和理由。"""
        ...
        try:
            ...
            with self.client.messages.stream(
                model="claude-sonnet-4-5-20250929",
                max_tokens=4000,
                system="你是一个专业的珠宝ERP系统分析专家，擅长从多个维度分析业务数据，进行智能计算和判断，提供有价值的洞察和建议。你能够理解业务逻辑，进行复杂的统计分析，并给出专业的建议。",
                messages=[{
                    "role": "user",
                    "content": prompt
                }]
