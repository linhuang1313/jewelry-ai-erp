...
def calculate_customer_net_gold(
    ...
    for rs in return_settlements_q.all():
        if rs.payment_method == 'physical_gold':
            total_return_gold += float(rs.physical_gold_weight or 0)
        elif rs.payment_method == 'mixed':
            total_return_gold += float(rs.gold_payment_weight or 0)
    ...
    logger.info(
        f"[金料余额明细] customer_id={customer_id}: "
        f"期初={initial_gold:.3f}, "
        f"来料={float(total_receipts):.3f}, "
        f"销退退料={total_return_gold:.3f}, "
        f"结算用料={total_settlement_gold:.3f}, "
        f"提料={float(total_withdrawals):.3f}, "
        f"存料结价={float(total_deposit_settled):.3f}, "
        f"净存料={net_gold:.3f}"
    )
...
def calculate_batch_net_gold(customer_ids: List[int], db: Session) -> Dict[int, float]:
    ...
    for row in return_settlements:
        ...
        if row.payment_method == 'physical_gold':
            return_gold_map[cid] += float(row.physical_gold_weight or 0)
        elif row.payment_method == 'mixed':
            return_gold_map[cid] += float(row.gold_payment_weight or 0)
    ...
    for row in settlements:
        ...
        if row.payment_method == 'physical_gold':
            settlement_gold_map[cid] += float(row.physical_gold_weight or 0)
        elif row.payment_method == 'mixed':
            settlement_gold_map[cid] += float(row.gold_payment_weight or 0)
