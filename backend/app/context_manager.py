...
def append_action(session_id: str, action: str, result: str = None, success: bool = True, data: Dict[str, Any] = None):
    """
    追加操作记录（Append-Only Context）
    保持上下文的连贯性
    
    Args:
        data: 操作结果的结构化数据（单号、商品名、克重等）
    """
    context = load_session_context(session_id)
    ...
    # 如果成功，同时更新 last_operation 实体
    if success and data:
        entities_update = {"last_action": action}
        for key in ["order_no", "order_id", "product_name", "weight", "labor_cost", "supplier", "customer_name"]:
            if key in data:
                entities_update[f"last_{key}"] = data[key]
        context["entities"].update(entities_update)
...
def generate_context_summary(session_id: str) -> str:
    ...
    if unresolved_errors:
        errors_text = [
            f"⚠️ {e.get('type', e.get('action', '未知'))}: {e.get('detail', e.get('error', ''))}"
            for e in unresolved_errors[-3:]
        ]
...
def cleanup_old_context_files(max_age_days: int = 7):
    """
    清理超过指定天数的 context 文件。
    内置节流：每 24 小时最多执行一次。
    """
    ...
    try:
        for f in os.listdir(CONTEXT_DIR):
            if not (f.startswith("session_") and f.endswith(".json")):
                continue
            ...
            try:
                ...
                continue
        if removed > 0:
            logger.info(f"[Context] 清理了 {removed} 个过期上下文文件")
    except Exception as e:
        logger.error(f"[Context] 清理上下文文件失败: {e}")
