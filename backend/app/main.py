...
async def startup_event():
    ...
    # 确保 User 表在 create_all 时被创建
    from .models.user import User  # noqa: F401 - 触发 User 模型注册到 Base
    ...
    _db_type = "PostgreSQL" if "postgresql" in _db_url else "SQLite" if "sqlite" in _db_url else "Unknown"
    logger.info(f"数据库类型: {_db_type}")

    # 初始化种子账号（7个角色默认账号）
    _seed_db = SessionLocal()
    try:
        from .routers.auth import init_seed_accounts
        from .models.user import User as _UserModel
        _user_count = _seed_db.query(_UserModel).count()
        logger.info(f"users 表现有 {_user_count} 条记录")
        _seed_count = init_seed_accounts(_seed_db)
        if _seed_count > 0:
            logger.info(f"已创建 {_seed_count} 个种子账号")
        else:
            logger.info("所有种子账号已存在，跳过创建")
    except Exception as e:
        logger.warning(f"种子账号初始化: {e}")
        _seed_db.rollback()
    finally:
        _seed_db.close()
