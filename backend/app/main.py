...
async def startup_event():
    ...
    try:
        # 获取所有客户
        customers = db.execute(text("SELECT id, name FROM customers WHERE status = 'active'")).fetchall()
        fixed_count = 0
        ...
        for customer in customers:
            ...
            if deposit_id:
                if old_balance > 0 and gold_debt > 0:
                    ...
                    logger.info(f"[金料账户修复] 客户 {customer_name}(ID:{customer_id}): "
        ...
        if fixed_count > 0:
            logger.info(f"[金料账户修复] 完成！共修复 {fixed_count} 个客户的金料账户数据")
        else:
            logger.info("[金料账户修复] 所有客户金料账户数据已是最新状态")
    except Exception as e:
        logger.warning(f"金料账户数据修复时出错: {e}")
        db.rollback()
    finally:
        db.close()
    ...
    try:
        ...
        if ai_response.action == "入库":
            ...
            if any(word in user_msg_lower for word in ["你好", "hi", "hello", "嗨", "hey"]):
                chat_response = f"{time_greeting}我是珠宝ERP智能助手，有什么可以帮您的吗？\n\n您可以告诉我：\n• 入库商品（如：古法戒指100克 工费8元 供应商金源珠宝）\n• 查询库存\n• 开销售单\n• 查询客户欠款"
            elif any(word in user_msg_lower for word in ["谢谢", "感谢", "thanks", "thank"]):
            ...
            elif any(word in user_msg_lower for word in ["好的", "收到", "明白", "知道了", "ok", "好"]):
                chat_response = "好的，有需要再叫我 👍"
            else:
                chat_response = f"抱歉，我没有完全理解您的意思。我是珠宝ERP助手，可以帮您：\n\n📦 入库管理（商品入库、查询入库单）\n📊 库存查询（查看库存、库存分析）\n🧾 销售开单（创建销售单、查询销售）\n💰 财务管理（客户欠款、收款登记）\n\n请告诉我具体需要什么帮助？"
...
# ==================== 销售单管理API ====================
# 销售单创建 API 已移至 routers/sales.py，包含库存扣减逻辑
...
async def handle_return(ai_response, db: Session, user_role: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if return_type == "to_supplier" and supplier_id:
            ...
            logger.info(f"供应商金料账户更新: 供应商={supplier_name}, 退货={weight}克, "
