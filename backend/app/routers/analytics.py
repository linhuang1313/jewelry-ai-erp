...
async def get_salesperson_performance(
    ...
    try:
        ...
        salespersons = []
        for idx, r in enumerate(results, 1):
            avg_order = float(r.total_amount or 0) / int(r.order_count) if r.order_count else 0
            salespersons.append({
                "rank": idx,
...
async def get_customer_analysis(
    ...
    try:
        ...
        customers = []
        for r in results:
            avg_amount = float(r.total_amount or 0) / int(r.purchase_count) if r.purchase_count else 0
            customers.append({
...
async def get_supplier_cost_analysis(
    ...
    try:
        ...
        for s in supplier_stats:
            unit_cost = float(s.total_cost or 0) / float(s.total_weight) if s.total_weight else 0
            suppliers.append({
                "supplier": s.supplier,
