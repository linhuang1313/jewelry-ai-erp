...
async def confirm_return_order(
    ...
    # 按退货类型检查权限
    if return_order.return_type == "to_supplier":
        if not has_permission(user_role, 'can_return_to_supplier'):
            raise HTTPException(status_code=403, detail="权限不足：无法确认退货单")
    elif return_order.return_type == "to_warehouse":
        if not has_permission(user_role, 'can_return_to_warehouse'):
            raise HTTPException(status_code=403, detail="权限不足：无法确认退货单")
