...
from ..models import Location, LocationInventory, InventoryTransfer, Inventory, InventoryTransferOrder, InventoryTransferItem, SalesOrder, SalesDetail, OrderStatusLog, ProductCode
...
async def get_location_inventory(
    ...
    include_available: bool = Query(False, description="展厅库存时返回可用克重（总库存 - draft 销售单占用）"),
    db: Session = Depends(get_db)
):
    ...
    # 展厅 + include_available：计算每商品 draft 占用
    reserved_by_product = {}
    show_available = False
    if include_available and items:
        showroom_location = db.query(Location).filter(Location.code == "showroom", Location.is_active == 1).first()
        is_showroom = showroom_location and (
            (location_id and location_id == showroom_location.id) or
            any(it.location and it.location.code == "showroom" for it in items)
        )
        if is_showroom:
            show_available = True
            product_names = [it.product_name for it in items]
            reserved_rows = db.query(
                SalesDetail.product_name,
                func.sum(SalesDetail.weight).label("reserved")
            ).join(SalesOrder).filter(
                SalesDetail.product_name.in_(product_names),
                SalesOrder.status == "draft"
            ).group_by(SalesDetail.product_name).all()
            reserved_by_product = {r.product_name: float(r.reserved or 0) for r in reserved_rows}
    
    # 查询 product_codes 构建 name -> code 映射（predefined 优先）
    product_names_list = list(set(item.product_name for item in items))
    code_map: dict[str, str] = {}
    if product_names_list:
        code_rows = db.query(ProductCode.name, ProductCode.code, ProductCode.code_type).filter(
            ProductCode.name.in_(product_names_list)
        ).all()
        for name, code, ct in code_rows:
            if name not in code_map or ct == 'predefined':
                code_map[name] = code
    ...
    for item in items:
        item_dict = {
            ...
            "product_code": code_map.get(item.product_name),
        ...
        if show_available:
            reserved = reserved_by_product.get(item.product_name, 0)
            item_dict["available_weight"] = max(0, float(item.weight or 0) - reserved)
...
async def get_inventory_summary(
    product_name: Optional[str] = None,
    limit: int = Query(default=200, ge=1, le=5000, description="返回记录数限制"),
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        if product_names:
            ...
            for stat in stats_query:
                product_stats[stat.product_name] = {
                    ...
                    "total_amount": float(stat.total_amount) if stat.total_amount is not None else 0.0
                }
        ...
        if product_names:
            code_rows = db.query(ProductCode.name, ProductCode.code, ProductCode.code_type).filter(
                ProductCode.name.in_(product_names)
            ).all()
            for row in code_rows:
                if row.code_type == 'predefined' or row.name not in code_map:
                    code_map[row.name] = row.code
        
        summary = {}
        for item in items:
            if item.product_name not in summary:
                stats = product_stats.get(item.product_name, {"quantity": 0, "total_amount": 0.0})
            ...
            summary[item.product_name]["total_weight"] += float(item.weight or 0)
        ...
        logger.error(f"获取库存汇总失败: {e}", exc_info=True)
...
async def create_transfer(
    ...
    role: str = Depends(require_permission("can_transfer")),
    db: Session = Depends(get_db)
):
    """创建货品转移单"""
    # 权限检查
    ...
    # 扣减发出位置库存
    from_inventory.weight = round(float(from_inventory.weight or 0) - float(transfer.weight or 0), 4)
...
async def create_batch_transfers(
    ...
    for item in data.items:
        try:
            ...
            # 扣减库存（行已锁定）
            inventory.weight = round(float(inventory.weight or 0) - float(item.weight or 0), 4)
...
async def receive_transfer(
    ...
    user_role: str = Query(default="sales", description="用户角色"),
    role: str = Depends(require_permission("can_receive_transfer")),
    db: Session = Depends(get_db)
):
    """接收货品转移
    ...
    if abs(weight_diff) < 0.01:
        ...
        if to_inventory:
            to_inventory.weight = round(float(to_inventory.weight or 0) + float(receive_data.actual_weight or 0), 4)
        else:
            to_inventory = LocationInventory(
                product_name=transfer.product_name,
                location_id=transfer.to_location_id,
                weight=float(receive_data.actual_weight or 0)
            )
            db.add(to_inventory)
...
async def confirm_transfer(
    ...
    if to_inventory:
        to_inventory.weight = round(float(to_inventory.weight or 0) + float(transfer.actual_weight or 0), 4)
    else:
        to_inventory = LocationInventory(
            product_name=transfer.product_name,
            location_id=transfer.to_location_id,
            weight=float(transfer.actual_weight or 0)
        )
        db.add(to_inventory)
...
async def reject_confirm_transfer(
    ...
    if from_inventory:
        ...
        from_inventory = LocationInventory(
            ...
            weight=float(transfer.weight or 0)
        )
...
async def get_inventory_overview(
    ...
    """
    # 检查缓存
    cache_key = f"overview_{user_role}"
    now = _time.time()
    ...
    # 写入缓存
    _OVERVIEW_CACHE[cache_key] = (response_data, _time.time())
...
async def create_transfer_order(
    ...
    for item in data.items:
        ...
        if not inventory or inventory.weight < item.weight:
            ...
            locked_inventories[item.product_name] = inventory
    
    if errors:
        raise HTTPException(status_code=400, detail="; ".join(errors))
    
    # 查询商品编码映射
    product_names_list = list(set(item.product_name for item in data.items))
    code_map: dict[str, str] = {}
    if product_names_list:
        code_rows = db.query(ProductCode.name, ProductCode.code, ProductCode.code_type).filter(
            ProductCode.name.in_(product_names_list)
    ...
    total_weight = 0
    for item in data.items:
        inventory = locked_inventories[item.product_name]
...
async def receive_transfer_order(
    order_id: int,
    receive_data: Optional[TransferOrderReceive] = None,
    ...
    柜台直接点击"确认接收"，无需填写实际重量。
    实际重量默认等于预期重量，库存直接转入目标位置。
    如果有重量问题，柜台线下告知商品部，商品部在"待确认"中编辑。
    """
    from ..middleware.permissions import has_permission
    if not has_permission(user_role, 'can_receive_transfer'):
        raise HTTPException(status_code=403, detail="权限不足：您没有【接收库存】的权限")
    ...
    try:
        # 简化流程：直接按预期重量接收，actual_weight = weight
        ...
        log = OrderStatusLog(
            ...
            new_status="received",
            operated_by=received_by,
            operated_at=china_now(),
            remark=f"柜台确认接收，共 {len(order.items)} 个商品"
        )
        db.add(log)
        ...
        db.rollback()
        logger.error(f"接收转移单失败: {e}")
        raise HTTPException(status_code=500, detail=f"接收失败: {str(e)}")
...
async def reject_transfer_order(
    ...
    log = OrderStatusLog(
        ...
        old_status=old_status,
        new_status="rejected",
        operated_by=rejected_by,
        operated_at=china_now(),
        remark=f"拒收原因: {reason}"
    )
    db.add(log)
...
async def confirm_transfer_order(
    ...
    confirm_log = OrderStatusLog(
        ...
        operated_at=china_now(),
        remark=f"商品部确认转移单，共 {len(order.items)} 个商品"
    )
    db.add(confirm_log)
...
async def update_transfer_order_actual(
    ...
    支持 pending_confirm 和 rejected 状态的编辑。
    编辑时自动同步库存差额（新旧重量差退回/扣除商品部库存），并记录操作日志。
    如果转移单状态为 rejected，编辑后自动恢复为 pending_confirm。
    """
    from ..middleware.permissions import has_permission
    if not has_permission(user_role, 'can_transfer'):
    ...
    if not order:
        raise HTTPException(status_code=404, detail="转移单不存在")
    if order.status not in ("pending_confirm", "rejected"):
    ...
    for item in order.items:
        if item.id not in update_map:
            raise HTTPException(status_code=400, detail=f"缺少商品 {item.product_name} 的数据")
    ...
    try:
        for item in order.items:
            ...
            if update_item.actual_weight <= 0:
                raise HTTPException(status_code=400, detail=f"商品 {item.product_name} 重量必须大于 0")
            
            old_weight = item.weight
            new_weight = update_item.actual_weight
            weight_diff = new_weight - old_weight
            
            # 库存同步：如果重量有变化，调整商品部库存
            if abs(weight_diff) >= 0.001:
                ...
                if weight_diff < 0:
                    ...
                    # 新重量更多，从商品部额外扣除
                    if not from_inventory or from_inventory.weight < weight_diff:
                        available = from_inventory.weight if from_inventory else 0
                        raise HTTPException(
                            status_code=400, 
            ...
            item.actual_weight = new_weight  # 同步actual_weight
            item.weight_diff = 0  # 编辑后差异清零
            item.diff_reason = update_item.diff_reason
        
        # 如果是 rejected 状态，编辑后恢复为 pending_confirm
        if old_status == "rejected":
            order.status = "pending_confirm"
            # 注意：rejected时库存已恢复到商品部，现在重新扣除
            for item in order.items:
                from_inventory = db.query(LocationInventory).filter(
                    LocationInventory.location_id == order.from_location_id,
                    LocationInventory.product_name == item.product_name
        ...
        log_remark = "编辑转移单"
        if remark_parts:
            log_remark += "，重量变更: " + "; ".join(remark_parts)
        if old_status == "rejected":
            log_remark += "（从拒收状态恢复）"
        ...
        db.rollback()
        logger.error(f"编辑转移单失败: {e}")
        raise HTTPException(status_code=500, detail=f"编辑失败: {str(e)}")
    
    logger.info(f"编辑转移单: {order.transfer_no}, {log_remark}")
...
@router.delete("/transfer-orders/{order_id}")
async def delete_transfer_order(
    order_id: int,
    deleted_by: str = Query(default="系统管理员", description="删除人"),
    user_role: str = Query(default="sales", description="用户角色"),
    reason: str = Query(default="", description="删除原因"),
    db: Session = Depends(get_db)
):
    """删除转移单（商品专员操作）
    
    仅允许删除 pending_confirm 和 rejected 状态的转移单。
    删除后恢复商品部库存（rejected 状态的单库存已恢复，pending_confirm 的需要恢复），并记录日志。
    """
    from ..middleware.permissions import has_permission
    if not has_permission(user_role, 'can_transfer'):
        raise HTTPException(status_code=403, detail="权限不足：您没有删除转移单的权限")
    ...
    if not order:
        raise HTTPException(status_code=404, detail="转移单不存在")
    if order.status not in ("pending_confirm", "rejected"):
        raise HTTPException(status_code=400, detail=f"转移单状态为 {order.status}，无法删除")
    
    try:
        inventory_details = []
        
        # 如果是 pending_confirm 状态，需要恢复商品部库存（rejected 状态下库存已在拒收时恢复）
        if order.status == "pending_confirm":
            for item in order.items:
                from_inventory = db.query(LocationInventory).filter(
                    LocationInventory.location_id == order.from_location_id,
            ...
            # rejected 状态，库存已恢复，只记录明细
            for item in order.items:
                inventory_details.append(f"{item.product_name} {item.weight}g")
        
        # 记录操作日志
        log_remark = f"删除转移单 {order.transfer_no}"
        if reason:
            log_remark += f"，原因: {reason}"
        log_remark += f"，商品明细: {'; '.join(inventory_details)}"
        if order.status == "pending_confirm":
            log_remark += "（库存已恢复至商品部）"
        ...
        logger.info(f"删除转移单: {transfer_no}, {log_remark}")
        
        return {"success": True, "message": f"转移单 {transfer_no} 已删除", "details": log_remark}
    except HTTPException:
        db.rollback()
        raise
    except Exception as e:
        db.rollback()
        logger.error(f"删除转移单失败: {e}")
...
async def migrate_old_transfers(
    ...
    # 构建商品编码映射
    all_names = list(set(t.product_name for t in old_transfers))
    code_map: dict[str, str] = {}
    if all_names:
        code_rows = db.query(ProductCode.name, ProductCode.code, ProductCode.code_type).filter(
            ProductCode.name.in_(all_names)
        ).all()
        for row in code_rows:
            if row.code_type == 'predefined' or row.name not in code_map:
                code_map[row.name] = row.code
    ...
    for transfer in old_transfers:
        ...
        item = InventoryTransferItem(
            order_id=order.id,
            product_name=transfer.product_name,
            product_code=code_map.get(transfer.product_name),
            weight=transfer.weight,
            actual_weight=transfer.actual_weight,
            weight_diff=transfer.weight_diff,
            diff_reason=transfer.diff_reason
...
async def migrate_add_source_order_id(
    user_role: str = Query(default="sales"),
    db: Session = Depends(get_db)
):
