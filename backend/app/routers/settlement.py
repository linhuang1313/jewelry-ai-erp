...
def _generate_sequential_no(db: Session, model, column, prefix: str, max_retries: int = 3) -> str:
    """
    Concurrency-safe sequential order number generator.
    Locks the row with the highest number via ORDER BY DESC LIMIT 1 FOR UPDATE,
    then increments. Falls back to timestamp-based suffix on repeated collision.
    """
    from ..timezone_utils import china_now as _cn
    now = _cn()
    ...
    for attempt in range(max_retries):
        ...
        order_no = f"{full_prefix}{seq:03d}"

        existing = db.query(model).filter(column == order_no).first()
        if existing is None:
            return order_no

        logger.warning(
            "Order number collision: %s (attempt %d/%d)",
            order_no, attempt + 1, max_retries,
        )
...
def _build_detail_responses(details: list, db: Session) -> list:
    """构建 SalesDetailResponse 列表，F 码产品附加镶嵌字段"""
...
async def get_pending_sales_orders(
    ...
    # 查找状态为"待结算"或"confirmed"（兼容历史数据）的销售单
    orders = db.query(SalesOrder).filter(
        SalesOrder.status.in_(["待结算", "confirmed"])
    ...
    for order in orders:
        if order.id not in settlement_set:
            ...
            order_dict = {
                ...
                "details": _build_detail_responses(details, db)
            }
...
async def get_settlement_orders(
    ...
    status_counts = query.with_entities(
        func.count(case((SettlementOrder.status.in_(['pending', 'draft']), 1))).label('pending_count'),
        func.count(case((SettlementOrder.status.in_(['confirmed', 'printed']), 1))).label('completed_count'),
    ).first()
    pending_count = status_counts.pending_count if status_counts else 0
    completed_count = status_counts.completed_count if status_counts else 0
    
    # 分页
    offset = (page - 1) * page_size
    orders = query.order_by(SettlementOrder.created_at.desc()).offset(offset).limit(page_size).all()
    ...
    # 4. 构建响应（不再触发额外查询，容忍迁移数据中的 NULL 值）
    result = []
    for order in orders:
        ...
        if sales_order:
            try:
                sales_order_response = SalesOrderResponse(
                    id=sales_order.id,
                    order_no=sales_order.order_no or "",
                    order_date=sales_order.order_date,
                    customer_name=sales_order.customer_name or "",
                    salesperson=sales_order.salesperson or "",
                    store_code=sales_order.store_code,
                    total_labor_cost=sales_order.total_labor_cost or 0.0,
                    total_weight=sales_order.total_weight or 0.0,
                    remark=sales_order.remark,
                    status=sales_order.status or "unknown",
                    create_time=sales_order.create_time,
                    operator=sales_order.operator or "",
                    details=_build_detail_responses(details, db)
                )
            except Exception as e:
                logger.warning(f"构建销售单响应失败(id={sales_order.id}): {e}")
        
        try:
            result.append(SettlementOrderResponse(
                id=order.id,
                settlement_no=order.settlement_no or "",
                sales_order_id=order.sales_order_id,
                payment_method=order.payment_method or "cash_price",
                gold_price=order.gold_price,
                physical_gold_weight=order.physical_gold_weight,
                total_weight=order.total_weight or 0.0,
                material_amount=order.material_amount or 0.0,
                labor_amount=order.labor_amount or 0.0,
            ...
            logger.warning(f"构建结算单响应失败(id={order.id}): {e}")
...
async def create_settlement_order(
    ...
    role: str = Depends(require_permission("can_create_settlement")),
    db: Session = Depends(get_db)
):
    """
    创建结算单（结算专员创建）
    ...
    # 校验销售单状态：只有已确认的销售单才能创建结算单
    if sales_order.status not in ["confirmed", "待结算"]:
        raise HTTPException(
            status_code=400,
            detail=f"销售单状态为「{sales_order.status}」，只有已确认的销售单才能创建结算单"
        )
    
    # 加行锁检查幂等性：防止并发创建重复结算单
    existing = db.query(SettlementOrder).filter(
        SettlementOrder.sales_order_id == data.sales_order_id,
        SettlementOrder.status != "cancelled"
    ).with_for_update().first()
    if existing:
        raise HTTPException(status_code=409, detail="该销售单已有结算单（可能由其他人同时创建）")
    ...
    if data.payment_method == "cash_price":
        ...
        if weight_difference < -0.01 and not data.confirmed_underpay:
            raise HTTPException(
                status_code=400,
                detail=f"支付不足：支付克重({total_input:.2f}克) 小于 应付克重({so_total_weight:.2f}克)，差额{abs(weight_difference):.2f}克。请确认后重试。"
            )
    ...
    # 生成结算单号（并发安全）
    now = china_now()
    settlement_no = _generate_sequential_no(db, SettlementOrder, SettlementOrder.settlement_no, "JS")
    ...
    # 注意：金料扣减已移至确认结算单时执行（draft状态不影响客户账户）
...
async def update_settlement_order(
    ...
    if data.gold_price is not None:
        if data.gold_price < 0:
            raise HTTPException(status_code=400, detail="金价不能为负数")
    ...
    if payment_method == "cash_price":
        if gold_price <= 0:
            raise HTTPException(status_code=400, detail="结价支付需要填写当日金价（金价必须大于0）")
        ...
        if not physical_gold_weight:
            physical_gold_weight = total_weight
        ...
        settlement.physical_gold_weight = gold_payment_weight
        
        material_amount = cash_payment_weight * gold_price
        total_amount = material_amount + labor_amount
    else:
...
async def confirm_settlement_order(
    ...
    from ..middleware.permissions import has_permission
    if not has_permission(user_role, 'can_create_settlement'):
        raise HTTPException(status_code=403, detail="权限不足：您没有【确认结算单】的权限")
    
    # 加行锁防止并发重复确认
    settlement = db.query(SettlementOrder).filter(SettlementOrder.id == settlement_id).with_for_update().first()
    if not settlement:
        raise HTTPException(status_code=404, detail="结算单不存在")
    
    if settlement.status != "draft":
        raise HTTPException(status_code=409, detail=f"结算单状态为 {settlement.status}，无法确认（可能已被其他人确认）")
    
    # 锁定关联销售单防止并发修改
    sales_order = db.query(SalesOrder).filter(SalesOrder.id == settlement.sales_order_id).with_for_update().first()
    ...
    if settlement.payment_method in ["physical_gold", "mixed"] and customer_id and actual_gold_due > 0:
        ...
        now_confirm = china_now()
        # 锁定客户存料记录防止并发余额修改
        customer_deposit = db.query(CustomerGoldDeposit).filter(CustomerGoldDeposit.customer_id == customer_id).with_for_update().first()
        ...
        deposit_transaction = CustomerGoldDepositTransaction(
            ...
            remark=f"结算单：{settlement.settlement_no}，结料支付"
        )
        ...
        if settlement.payment_status == "underpaid" and settlement.payment_difference and float(settlement.payment_difference) < -0.01:
            ...
            underpay_tx = CustomerGoldDepositTransaction(
                ...
                transaction_type='use',
                settlement_order_id=settlement.id,
                amount=underpay_weight,
                balance_before=balance_before_underpay,
                balance_after=customer_deposit.current_balance,
                created_by=data.confirmed_by,
                remark=f"结算单：{settlement.settlement_no}，少付差额{underpay_weight:.2f}克记为金料欠款"
            )
            db.add(underpay_tx)
            logger.info(f"确认结算-少付差额: 客户={customer_name}, 差额={underpay_weight}克记为欠料, {balance_before_underpay:.2f} -> {customer_deposit.current_balance:.2f}克")
    # ========== 金料扣减结束 ==========
    ...
    try:
        ...
        logger.warning(f"记录决策日志失败(不影响业务): {e}")
    
    logger.info(f"确认结算单: {settlement.settlement_no}, 确认人: {data.confirmed_by}")
    
    return SettlementOrderResponse(
        id=settlement.id,
        settlement_no=settlement.settlement_no or "",
        sales_order_id=settlement.sales_order_id,
        payment_method=settlement.payment_method or "cash_price",
        gold_price=settlement.gold_price,
        physical_gold_weight=settlement.physical_gold_weight,
        total_weight=settlement.total_weight or 0.0,
        material_amount=settlement.material_amount or 0.0,
        labor_amount=settlement.labor_amount or 0.0,
        total_amount=settlement.total_amount or 0.0,
        gold_payment_weight=settlement.gold_payment_weight,
        cash_payment_weight=settlement.cash_payment_weight,
        previous_cash_debt=settlement.previous_cash_debt or 0.0,
        previous_gold_debt=settlement.previous_gold_debt or 0.0,
        gold_deposit_balance=settlement.gold_deposit_balance or 0.0,
        cash_deposit_balance=settlement.cash_deposit_balance or 0.0,
        status=settlement.status or "confirmed",
        created_by=settlement.created_by,
        confirmed_by=settlement.confirmed_by,
        confirmed_at=settlement.confirmed_at,
        printed_at=settlement.printed_at,
...
async def get_settlement_order(
    ...
    settlement = db.query(SettlementOrder).filter(
        ...
        joinedload(SettlementOrder.sales_order).selectinload(SalesOrder.details)
    ).first()
    if not settlement:
        raise HTTPException(status_code=404, detail="结算单不存在")
    
    sales_order = settlement.sales_order
    details = sales_order.details if sales_order else []
    ...
    return SettlementOrderResponse(
        ...
        gold_deposit_balance=settlement.gold_deposit_balance or 0.0,
        cash_deposit_balance=settlement.cash_deposit_balance or 0.0,
        status=settlement.status or "draft",
        created_by=settlement.created_by,
        confirmed_by=settlement.confirmed_by,
        confirmed_at=settlement.confirmed_at,
        printed_at=settlement.printed_at,
    ...
    try:
        ...
        if format == "pdf":
            try:
                ...
                # 第三行：结算备注（与销售单备注同一含义，不重复显示）
                y -= 9
                remark_text = (settlement.remark or (sales_order.remark if sales_order else None) or store_code or "")[:40]
                p.drawString(left_margin, y, f"结算备注：{remark_text}")
                ...
                for detail in details:
                    ...
                    total_labor_cost = float(detail.total_labor_cost or 0)
                ...
                pdf_total_amount = float(settlement.total_amount or 0)
                p.drawRightString(right_margin - 50*mm, y, f"结算金额：{pdf_total_amount:,.2f}")
                p.drawRightString(right_margin, y, f"本单应收：{pdf_total_amount:,.2f}")
                ...
                # 汇总数据（Decimal → float 转换）
                prev_cash = float(settlement.previous_cash_debt or 0)
                prev_gold = float(settlement.previous_gold_debt or 0)
                current_cash = float(settlement.total_amount or 0)
                current_gold = float(settlement.physical_gold_weight or 0)
            ...
            # 结算明细表格（结料/结价部分）
            # Decimal → float 转换 + None 防护
            s_total_weight = float(settlement.total_weight or 0)
            s_labor_amount = float(settlement.labor_amount or 0)
            s_gold_price = float(settlement.gold_price or 0)
            s_material_amount = float(settlement.material_amount or 0)
            s_labor_per_gram = s_labor_amount / s_total_weight if s_total_weight > 0 else 0
            ...
            if settlement.payment_method == "physical_gold":
                ...
                <tr>
                    ...
                    <td>出货结料</td>
                    <td>足金</td>
                    <td class="right">{s_total_weight:.3f}</td>
                    <td>足金</td>
                    <td class="right">{s_total_weight:.3f}</td>
                    <td class="right">{s_total_weight:.3f}</td>
                    <td class="right"></td>
                    <td class="right"></td>
                    <td class="right"></td>
                    <td class="right">{s_labor_per_gram:.2f}</td>
                    <td class="right"></td>
                    <td class="right">{s_labor_amount:.2f}</td>
                </tr>'''
                ...
                <tr>
                    ...
                    <td class="right"></td>
                    <td class="right">{s_total_weight:.3f}</td>
                    <td class="right">{s_gold_price:.2f}</td>
                    <td class="right">{s_material_amount:.2f}</td>
                    <td class="right">{s_labor_per_gram:.2f}</td>
                    <td class="right"></td>
                    <td class="right">{s_labor_amount:.2f}</td>
                </tr>'''
                ...
                # 混合支付 - 分两行显示
                gold_weight = float(settlement.gold_payment_weight or 0)
                cash_weight = float(settlement.cash_payment_weight or 0)
                gold_labor = gold_weight * s_labor_per_gram
                cash_labor = cash_weight * s_labor_per_gram
                cash_material = cash_weight * s_gold_price
...
<body>
    <div class="page">
        ...
        <div class="info-row">
            ...
            <span class="item">结算备注：{html.escape(settlement.remark or (sales_order.remark if sales_order else None) or store_code or '')}</span>
            </div>
        ...
        <div class="summary-row">
            <span>优惠额：0.00</span>
            <span>结算金额：<b>{current_cash:,.2f}</b></span>
            <span>本单应收：<b>{current_cash:,.2f}</b></span>
            </div>
...
async def cancel_settlement_order(
    ...
    try:
        ...
        # draft状态取消不需要回滚金料（金料扣减在确认时才执行）
        sales_order = db.query(SalesOrder).filter(SalesOrder.id == settlement.sales_order_id).first()
        
        # 将结算单状态改为 cancelled
        settlement.status = "cancelled"
        
        # 销售单状态改回已确认（允许重新创建结算单）
        ...
        msg = f"结算单 {settlement.settlement_no} 已取消"
...
async def revert_settlement_order(
    ...
    if settlement.status not in ["confirmed", "printed"]:
        raise HTTPException(status_code=409, detail=f"结算单状态为 {settlement.status}，无法撤销（可能已被其他人操作）")
    ...
    try:
        ...
        if cancelled_cash > 0:
            logger.info(f"撤销结算：取消应收账款未收部分 {cancelled_cash:.2f}元")
        
        # 如果有已收款，转为客户预收款（不是退款，是账上余额）
        if total_received > 0 and customer_id_for_prepayment:
            prepayment_tx = CustomerTransaction(
                customer_id=customer_id_for_prepayment,
                customer_name=sales_order.customer_name,
                transaction_type='prepayment',
                settlement_order_id=settlement.id,
                amount=total_received,
                status='active',
                remark=f"撤销结算转预收款：结算单 {settlement.settlement_no}，原已收 {total_received:.2f}元 转为预收款"
            )
            db.add(prepayment_tx)
            logger.info(f"撤销结算：已收款 {total_received:.2f}元 转为客户预收款")
        ...
        if settlement.payment_method in ["physical_gold", "mixed"] and customer_id:
            deposit_txs = db.query(CustomerGoldDepositTransaction).filter(
                CustomerGoldDepositTransaction.settlement_order_id == settlement.id,
                CustomerGoldDepositTransaction.transaction_type == 'use',
                CustomerGoldDepositTransaction.status == 'active'
            ).all()
                ...
                for deposit_tx in deposit_txs:
                    ...
                    if gold_to_rollback > 0:
                        ...
                        rollback_tx = CustomerGoldDepositTransaction(
                            ...
                            remark=f"撤销结算回滚：结算单 {settlement.settlement_no}，原记录：{deposit_tx.remark or ''}"
                        )
                        db.add(rollback_tx)
                        rolled_back_gold += gold_to_rollback
                        logger.info(f"撤销结算：回滚金料 {gold_to_rollback:.2f}克，余额 {balance_before:.2f} -> {customer_deposit.current_balance:.2f}")
...
async def refund_sales_order(
    ...
    try:
        ...
        for detail in details:
            # 生成退货单号（并发安全）
            return_no = _generate_sequential_no(db, ReturnOrder, ReturnOrder.return_no, "TH")
...
@router.post("/deposit-settlements", summary="创建存料结价单")
async def create_deposit_settlement(
    data: DepositSettlementCreate,
    user_role: str = Query("settlement", description="用户角色"),
    db: Session = Depends(get_db)
):
    """创建存料结价单（草稿状态）"""
    from ..middleware.permissions import has_permission
    if not has_permission(user_role, 'can_create_settlement'):
        raise HTTPException(status_code=403, detail="无权限创建存料结价单")
    ...
    # 使用统一计算函数获取存料余额（保证全系统口径一致）
    from ..gold_balance import calculate_customer_net_gold
    current_balance = calculate_customer_net_gold(data.customer_id, db)  # 正数=存料
    
    if data.gold_weight <= 0:
        raise HTTPException(status_code=400, detail="结价克重必须大于0")
    if data.gold_price <= 0:
        raise HTTPException(status_code=400, detail="金价必须大于0")
    if data.gold_weight > current_balance:
        raise HTTPException(
            status_code=400, 
            detail=f"结价克重({data.gold_weight:.3f}g)超过存料余额({current_balance:.3f}g)"
        )
    ...
    logger.info(f"创建存料结价单 {settlement_no}: 客户={customer.name}, {data.gold_weight}g × ¥{data.gold_price} = ¥{total_amount}")
    
    return {
        "success": True,
        "message": f"存料结价单 {settlement_no} 创建成功",
        "data": {
...
@router.post("/deposit-settlements/{settlement_id}/confirm", summary="确认存料结价单")
async def confirm_deposit_settlement(
    settlement_id: int,
    user_role: str = Query("settlement", description="用户角色"),
    db: Session = Depends(get_db)
):
    """确认存料结价单：扣减存料余额 + 创建来款记录抵扣欠款"""
    from ..middleware.permissions import has_permission
    if not has_permission(user_role, 'can_create_settlement'):
        raise HTTPException(status_code=403, detail="无权限确认存料结价单")
    
    # 加行锁防止并发确认
    settlement = db.query(DepositSettlement).filter(
        DepositSettlement.id == settlement_id
    ).with_for_update().first()
    if not settlement:
        raise HTTPException(status_code=404, detail="存料结价单不存在")
    if settlement.status != "draft":
        raise HTTPException(status_code=409, detail=f"当前状态为{settlement.status}，只有草稿状态可确认（可能已被其他人确认）")
    
    # 使用统一计算函数验证存料余额（保证全系统口径一致）
    from ..gold_balance import calculate_customer_net_gold
    net_gold = calculate_customer_net_gold(settlement.customer_id, db)  # 正数=存料
    if net_gold <= 0:
        raise HTTPException(status_code=400, detail=f"客户无存料余额（当前{net_gold:.3f}g）")
    if net_gold < settlement.gold_weight:
        raise HTTPException(
            status_code=400,
            detail=f"存料余额不足：当前{net_gold:.3f}g，需要{settlement.gold_weight:.3f}g"
        )
    
    # 锁定客户存料记录防止并发余额修改
    deposit = db.query(CustomerGoldDeposit).filter(
        CustomerGoldDeposit.customer_id == settlement.customer_id
    ).with_for_update().first()
    if not deposit:
        from .gold_material import get_or_create_customer_deposit
        deposit = get_or_create_customer_deposit(settlement.customer_id, settlement.customer_name, db)
    ...
    try:
        ...
        deposit_tx = CustomerGoldDepositTransaction(
            ...
            transaction_type='use',
            amount=-settlement.gold_weight,
            balance_before=balance_before,
            balance_after=deposit.current_balance,
            status='active',
            created_by=user_role,
            remark=f"存料结价 {settlement.settlement_no}：{settlement.gold_weight:.3f}g × ¥{settlement.gold_price:.0f}/g = ¥{settlement.total_amount:,.2f}"
        )
        db.add(deposit_tx)
        
        # 3. 创建来款记录（PaymentRecord）冲抵欠款
        payment_no = _generate_sequential_no(db, PaymentRecord, PaymentRecord.payment_no, "SK")
        ...
        payment = PaymentRecord(
            ...
            remark=f"存料结价抵扣：{settlement.settlement_no}，{settlement.gold_weight:.3f}g × ¥{settlement.gold_price:.0f}/g",
            status='confirmed',
            operator=user_role,
        )
        db.add(payment)
        ...
        # 4. 锁定并冲抵应收账款
        receivables = db.query(AccountReceivable).filter(
            AccountReceivable.customer_id == settlement.customer_id,
            AccountReceivable.status.in_(['unpaid', 'overdue'])
        ...
        # 5. 更新存料结价单状态
        settlement.status = "confirmed"
        settlement.confirmed_by = user_role
        settlement.confirmed_at = now
        
        db.commit()
        
        logger.info(
            f"确认存料结价单 {settlement.settlement_no}: "
            f"扣减存料 {settlement.gold_weight}g, 抵扣欠款 ¥{settlement.total_amount}"
        )
        
        return {
            "success": True,
            "message": f"存料结价单 {settlement.settlement_no} 已确认，扣减存料 {settlement.gold_weight:.3f}g，抵扣欠款 ¥{settlement.total_amount:,.2f}",
            "data": {
                "id": settlement.id,
                "settlement_no": settlement.settlement_no,
                "gold_weight": settlement.gold_weight,
                "total_amount": settlement.total_amount,
                "deposit_balance_after": deposit.current_balance
            }
        }
        
    except Exception as e:
        db.rollback()
        logger.error(f"确认存料结价单失败: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"确认失败: {str(e)}")


@router.get("/deposit-settlements", summary="存料结价单列表")
async def list_deposit_settlements(
    ...
    """获取存料结价单列表"""
    query = db.query(DepositSettlement)
    if status:
        query = query.filter(DepositSettlement.status == status)
    if customer_name:
        query = query.filter(DepositSettlement.customer_name.contains(customer_name))
    
    settlements = query.order_by(DepositSettlement.created_at.desc()).limit(limit).all()
...
@router.post("/deposit-settlements/{settlement_id}/cancel", summary="取消存料结价单")
async def cancel_deposit_settlement(
    settlement_id: int,
    user_role: str = Query("settlement", description="用户角色"),
    db: Session = Depends(get_db)
):
    ...
    from ..middleware.permissions import has_permission
    if not has_permission(user_role, 'can_create_settlement'):
        raise HTTPException(status_code=403, detail="无权限取消存料结价单")
    ...
    if not settlement:
        raise HTTPException(status_code=404, detail="存料结价单不存在")
    if settlement.status != "draft":
        raise HTTPException(status_code=400, detail="只有草稿状态的存料结价单可以取消")
    
    settlement.status = "cancelled"
    db.commit()
    
    logger.info(f"取消存料结价单 {settlement.settlement_no}")
    
    return {
        "success": True,
        "message": f"存料结价单 {settlement.settlement_no} 已取消"
    }
