...
class AIRequest(BaseModel):
    ...
    image_url: Optional[str] = None  # base64 data URL of attached image (for OCR validation)
...
class AIResponse(BaseModel):
    ...
    # 退货相关字段（对话创建退货单）
    return_type: Optional[str] = None  # to_supplier / to_warehouse
    return_product_name: Optional[str] = None  # 退货商品名称
    return_weight: Optional[float] = None  # 退货克重
    return_labor_cost: Optional[float] = None  # 退货克工费
    return_supplier_name: Optional[str] = None  # 退给哪个供应商
    return_reason: Optional[str] = None  # 退货原因
    return_from_location: Optional[str] = None  # 发起位置
    # 确认/反确认单据相关字段
    confirm_order_type: Optional[str] = None  # inbound/sales/return/settlement
    confirm_order_no: Optional[str] = None  # 单据编号（RK.../XS.../TH.../JS...）
    # 销退相关字段
    sales_return_customer_name: Optional[str] = None
    sales_return_product_name: Optional[str] = None
    sales_return_weight: Optional[float] = None
    sales_return_reason: Optional[str] = None
    sales_return_order_no: Optional[str] = None
    # 存料结价相关字段
    deposit_settle_customer_name: Optional[str] = None  # 存料结价客户名称
    deposit_settle_gold_weight: Optional[float] = None  # 结价克重
    deposit_settle_gold_price: Optional[float] = None   # 金价（元/克）
    deposit_settle_remark: Optional[str] = None          # 备注
    # 结算单相关字段（自然语言创建结算单）
    settlement_customer_name: Optional[str] = None  # 结算客户名称
    settlement_sales_order_no: Optional[str] = None  # 关联销售单号
    settlement_payment_method: Optional[str] = None  # 结算方式：结价/结料/混合
    settlement_gold_price: Optional[float] = None  # 当日金价（结价时必填）
    settlement_order_no: Optional[str] = None  # 结算单号（查询用）
    settlement_remark: Optional[str] = None  # 结算备注
    # 暂借单相关字段（自然语言创建暂借单）
    loan_customer_name: Optional[str] = None  # 暂借客户名称
    loan_items: Optional[List[dict]] = None  # 暂借商品列表 [{product_name, weight, labor_cost}]
    loan_salesperson: Optional[str] = None  # 业务员
    loan_remark: Optional[str] = None  # 暂借备注
    loan_order_no: Optional[str] = None  # 暂借单号（查询用）
    loan_return_order_no: Optional[str] = None  # 归还关联的暂借单号
    # 对账单查询字段
    reconciliation_customer_name: Optional[str] = None  # 对账客户名称
    reconciliation_month: Optional[str] = None  # 对账月份（YYYY-MM）
    # 凭证查询字段
    voucher_query_type: Optional[str] = None  # 凭证类型筛选
    voucher_date_start: Optional[str] = None  # 凭证起始日期
    voucher_date_end: Optional[str] = None  # 凭证结束日期
    voucher_keyword: Optional[str] = None  # 凭证关键词（摘要搜索）
    # 费用报销字段
    expense_category: Optional[str] = None  # 费用类别
    expense_amount: Optional[float] = None  # 费用金额
    expense_description: Optional[str] = None  # 费用描述
    expense_remark: Optional[str] = None  # 费用备注
    # 查询金料记录字段
    gold_record_type: Optional[str] = None  # 查询类型：提料/收料/付料/全部
    gold_record_customer_name: Optional[str] = None  # 按客户筛选
    gold_record_date_start: Optional[str] = None  # 开始日期
    gold_record_date_end: Optional[str] = None  # 结束日期
...
class InboundOrderCreate(BaseModel):
    """创建入库单请求"""
    product_code: Optional[str] = None  # 商品编码（JPJZ, F00000001等）
    barcode: Optional[str] = None  # 条码号（可自定义）
    product_name: str
...
class SettlementOrderResponse(BaseModel):
    ...
    payment_method: Optional[str] = "cash_price"
    gold_price: Optional[float] = None
    physical_gold_weight: Optional[float] = None
    total_weight: Optional[float] = 0.0
    material_amount: Optional[float] = 0.0
    labor_amount: Optional[float] = 0.0
...
class SalesReturnDetailItem(BaseModel):
    """销退明细项"""
    product_code: Optional[str] = None
    product_name: str
    weight: float
...
class SalesReturnOrderCreate(BaseModel):
    ...
    salesperson: Optional[str] = None
    order_date: Optional[datetime] = None
    return_to: str = "showroom"  # showroom/warehouse
    return_reason: str  # 客户退货/质量问题/款式不符/尺寸不合/其他
    reason_detail: Optional[str] = None
    items: List[SalesReturnDetailItem]
    remark: Optional[str] = None


class SalesReturnSettlementCreate(BaseModel):
    """创建销退结算单请求"""
    sales_return_order_id: int
    payment_method: str  # cash_price/physical_gold/mixed
    gold_price: Optional[float] = None
    physical_gold_weight: Optional[float] = None
    gold_payment_weight: Optional[float] = None  # 混合退款
    cash_payment_weight: Optional[float] = None  # 混合退款
    remark: Optional[str] = None
...
class DepositSettlementCreate(BaseModel):
    """创建存料结价单"""
    customer_id: int
    gold_weight: float  # 结价克重
    gold_price: float   # 金价（元/克）
    remark: Optional[str] = None


class DepositSettlementResponse(BaseModel):
    """存料结价单响应"""
    model_config = ConfigDict(from_attributes=True)
    id: int
    settlement_no: str
...
class GoldPurchaseOrderCreate(BaseModel):
    supplier_id: int
    gold_weight: float
...
class GoldPurchaseOrderPrice(BaseModel):
    gold_price: float


class GoldPurchasePaymentCreate(BaseModel):
...
class GoldPurchaseOrderResponse(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    id: int
    order_no: str
    supplier_id: Optional[int] = None
    ...
    conversion_rate: float = 1.0
    settled_weight: Optional[float] = None
    gold_price: Optional[float] = None
...
class GoldPurchasePaymentResponse(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    id: int
...
__all__ = [
    ...
    # 销退模块
    'SalesReturnDetailItem',
    'SalesReturnOrderCreate',
    'SalesReturnSettlementCreate',
    # 存料结价
