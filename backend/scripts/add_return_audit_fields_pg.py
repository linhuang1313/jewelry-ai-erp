"""
为 return_orders 表补充财务审核字段 (PostgreSQL 版本)
用于生产环境数据库迁移
"""
import os
import sys
...
def main():
    ...
    if 'return_orders' not in tables:
        print("错误: return_orders 表不存在")
        return
    
    print("检查 return_orders 表的列...")
    
    with engine.connect() as conn:
        # 添加 is_audited 字段
        if not column_exists(inspector, "return_orders", "is_audited"):
        ...
        # 添加 audited_by 字段
        if not column_exists(inspector, "return_orders", "audited_by"):
        ...
        # 添加 audited_at 字段
        if not column_exists(inspector, "return_orders", "audited_at"):
        ...
        try:
            conn.execute(text("""
                CREATE INDEX IF NOT EXISTS ix_return_orders_is_audited 
                ON return_orders (is_audited)
            """))
            conn.commit()
            print("✓ 索引创建/已存在: ix_return_orders_is_audited")
